...
"""
Original Repository:
https://github.com/google-research/google-research/blob/master/smurf/smurf_utils.py
"""
...
def flow_to_warp(flow):
  ...
  height, width = tf.unstack(tf.shape(flow))[-3:-1]
  i_grid, j_grid = tf.meshgrid(
      tf.linspace(0.0, tf.cast(height, tf.float32) - 1.0, height),
      tf.linspace(0.0, tf.cast(width, tf.float32) - 1.0, width),
      indexing='ij')
...
def mask_invalid(coords, pad_h=0, pad_w=0):
  ...
  max_height = tf.cast(tf.shape(coords)[-3] - 1, tf.float32)
  max_width = tf.cast(tf.shape(coords)[-2] - 1, tf.float32)
  mask = tf.logical_and(
      tf.logical_and(coords[:, :, :, 0] >= tf.reshape(pad_h, [-1,1,1]),
                     coords[:, :, :, 0] <= max_height),
      tf.logical_and(coords[:, :, :, 1] >= tf.reshape(pad_w, [-1,1,1]),
                     coords[:, :, :, 1] <= max_width))
...
def compute_range_map(flow,
  ...
  # Get input shape.
  input_shape = tf.unstack(tf.shape(flow))
...
def unsupervised_loss(images,
  ...
  for key in flows:
    ...
    if full_size_images is not None:
      flow = flows[key][0]
      height = tf.shape(flow)[-3]
      width = tf.shape(flow)[-2]
      full_height = tf.shape(full_size_images)[-3]
      full_width = tf.shape(full_size_images)[-2]
      batch_size = tf.shape(flow)[0]

      flow = util.image.pad_bboxes(flow, tf.stack([crop_h + pad_h, crop_w + pad_w], axis=-1),
                                   target_size=tf.stack([full_height, full_width]))
      ...
      warped_image = util.image.crop_bboxes(warped_image, tf.stack([crop_h + pad_h, crop_w + pad_w], axis=-1),
                                   target_size=tf.stack([height, width]))
      ...
      #
      #     cv2.imshow(str(time_j), full_size_images[0,time_j].numpy()[...,[2,1,0]])
      #     cv2.imshow("wi", warped_image.numpy()[0][...,[2,1,0]])
      #     cv2.imshow("oi", images[0, time_i].numpy()[..., [2, 1, 0]])
      #     cv2.imshow("fi", visualize_flow(flow[0]).numpy()[..., [2, 1, 0]])
      #     cv2.waitKey()
      # except:
      #     pass

      valid_warp_mask = util.image.crop_bboxes(valid_warp_mask, tf.stack([crop_h + pad_h, crop_w + pad_w], axis=-1),
                                            target_size=tf.stack([height, width]))
    else:
...
def self_supervision_loss(teacher_flow, student_flow,
  ...
  t_h, t_w = tf.unstack(tf.shape(student_mask)[1:3])
  teacher_mask = selfsup_transform_fn(teacher_mask, t_h, t_w, crop_h, crop_w, is_flow=False)
  teacher_flow = selfsup_transform_fn(teacher_flow, t_h, t_w, crop_h, crop_w, is_flow=True)
